...
> Owner note: this repository is preserved as a DeFi security research reference. It contains the original Wintermute Alpha Challenge prompts plus added DKG monitoring tooling under `dkg-monitor/`. The challenge folders are useful future study material for vault accounting, DAO governance risk, stale oracle/AMM behavior, MEV, liquidation mechanics, and Solana validator analysis.

## Local Additions

- `dkg-monitor/` - TypeScript scripts for monitoring and backfilling WalletRegistry DKG events with `ts-node`.
- The numbered folders are retained as challenge references and research prompts.
